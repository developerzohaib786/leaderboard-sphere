            ...
            <div className="border-b bg-white shadow-sm">
                <div className="flex items-center justify-between p-4">
                    <div className="flex items-center gap-3">
                        <Image
                            src={GithubImg}
                            alt="GitHub Issues Room"
                            width={48}
                            height={48}
                            className="rounded-full"
                        />
                        <div>
                            <h2 className="font-semibold text-lg">GitHub Issues Room</h2>
                            <p className="text-sm text-gray-500">Discuss GitHub issues and collaborate on solutions.</p>
                        </div>
                    </div>
                        ...
                        return (
                            <div key={index} className={`mb-4 flex items-start gap-3 ${isOwnMessage ? 'flex-row-reverse' : 'flex-row'}`}>
                                ...
                                <div className={`flex-1 max-w-[70%] ${isOwnMessage ? 'flex flex-col items-end' : 'flex flex-col items-start'}`}>
                                    ...
                                    <div className={`inline-block rounded-lg shadow overflow-hidden ${isOwnMessage ? 'bg-blue-500 text-white' : 'bg-blue-100'}`}>
                                        {/* Reply preview */}
                                        {msg.replyToText && (
                                            <div className={`text-xs px-3 pt-3 pb-2 border-b ${isOwnMessage ? 'border-blue-400' : 'border-blue-300'}`}>
                                        ...
                                        {msg.rawFileUrl && (
                                            <div className={`px-3 py-2 ${msg.message ? 'border-b' : ''} ${isOwnMessage ? 'bg-blue-600 border-blue-400' : 'bg-blue-50 border-blue-200'}`}>
                                                <a
                                                    href={msg.rawFileUrl}
                                                    target="_blank"
